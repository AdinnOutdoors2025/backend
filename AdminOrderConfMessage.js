    ...
    return new Promise((resolve, reject) => {
        ...
        switch (templateId) {
            case "1007478982147905431": // User Template
                text = `Thank you for your order with Adinn Outdoors! We've received it successfully. Your order ID is ${variables.orderId}.`;
                break;
            case "1007197121174928712": // Admin Template - FIXED: Use template ID directly
                text = `New order received! Order ID: ${variables.orderId}. Customer: ${variables.customerName}. Amount: ₹${variables.amount}.`;
                break;
            default:
                text = variables.text || "";
    ...
    try {
        ...
        if (IS_PRODUCTION) {
            ...
            for (const adminPhone of ADMIN_PHONE_NUMBERS) {
                try {
                    await sendAdminSMS(adminPhone, "1007197121174928712", {
                        orderId: orderId,
                        customerName: client.name,
                        amount: formatIndianCurrency(parseFloat(client.paidAmount) || 0)
                    });
                    console.log(`Admin SMS sent successfully to ${adminPhone}`);
                } catch (smsError) {
                    console.error(`Failed to send admin SMS to ${adminPhone}:`, smsError);
                }
            }
        } else {
            // Log SMS information to console for testing
