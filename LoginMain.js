...
const sendUserEmailSafely = async (userMailOptions, adminMailOptions) => {
    ...
    try {
        ...
        const info = await transporter.sendMail(userMailOptions, adminMailOptions);
        console.log('User Email sent successfully:', info.messageId);
        return { sent: true, info: info };

    } catch (error) {
        console.log('User Email sending failed (non-critical):', error.message);
...
<li><strong>Registration Date:</strong> ${new Date().toLocaleString()}</li>
        ...
        const emailResult = await sendUserEmailSafely(userMailOptions, adminMailOptions);
...
// Enhanced email function that handles SMTP blocks gracefully
const sendOtpEmailSafely = async (mailOptions) => {
    // Don't even try to send email on Render to avoid timeouts
    if (process.env.RENDER || process.env.NODE_ENV === 'production') {
        console.log('SMTP disabled on Render - skipping email send');
        return { sent: false, reason: 'SMTP blocked on Render' };
    ...
    try {
        ...
        // Try to send email but don't wait for it
        const emailResult = await sendOtpEmailSafely(mailOptions);
