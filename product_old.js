...
mongoose
  .connect(
    ...
    // "mongodb+srv://uiuxdesigner_db_user:SOVp8PJJpjRWrY62@adinn.gaqpapj.mongodb.net/?appName=bigg_bosss"
...
app.post("/cart", async (req, res) => {
  try {
    console.log("Received cart item:", {
      userId: req.body.userId,
      productId: req.body.productId,
    ...
    // Check if item already exists for this user with same dates
    const existingItem = await cartData.findOne({
      userId: req.body.userId,
      productId: req.body.productId,
      startDate: req.body.startDate,
    ...
    const newItem = new cartData(req.body);
    const savedItem = await newItem.save();
    console.log("Cart item saved:", savedItem._id);
    res.status(201).json(savedItem);
  } catch (err) {
    console.error("Error adding to cart:", err);
    res.status(500).json({
